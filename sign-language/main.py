...
s2p_map = {k.lower():v for k,v in load_json_file("sign-language/src/sign_to_prediction_index_map.json").items()}
p2s_map = {v:k for k,v in load_json_file("sign-language/src/sign_to_prediction_index_map.json").items()}
encoder = lambda x: s2p_map.get(x.lower())
decoder = lambda x: p2s_map.get(x)

models_path = [
                'sign-language/models/islr-fp16-192-8-seed_all42-foldall-last.h5',
]
